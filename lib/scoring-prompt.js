...
function buildScoringPrompt(content, context, deterministicFindings) {
  ...
  const contentTypeNote = isLandingPage
    ? `\nCONTENT TYPE: landing-page
Landing page adjustments:
- Rhythm: Score 5+ if uniform short sentences match a headline+body section structure (intentional, not metronomic). Do not score higher than 6 unless there is genuine length variation.
- Authenticity: Score based on distinct brand personality vs generic corporate tone. Do not require personal/first-person voice. Do not score higher than 6 unless copy has genuine personality or takes stances.
- Trust: Judge based on whether claims are SPECIFIC AND VERIFIABLE (named customers, concrete numbers, named sources like G2/Forbes), NOT whether source URLs appear inline. Landing pages link to case studies elsewhere. Score 5+ if most metrics use real numbers attributed to named entities. Only penalize vague unverifiable claims, absolute superlatives without qualifier, or metrics with zero indication of origin.
- Directness and Density: score exactly as for prose.`
    : "\nCONTENT TYPE: prose";
...
content_type|${isLandingPage ? "landing-page" : "prose"}
directness|N
rhythm|N
trust|N
authenticity|N
